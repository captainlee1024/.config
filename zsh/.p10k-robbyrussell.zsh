...
() {
  ...
  typeset -g POWERLEVEL9K_SHORTEN_STRATEGY=truncate_to_unique
	# Color of the shortened directory segments. 截断目录的颜色
  typeset -g POWERLEVEL9K_DIR_SHORTENED_FOREGROUND=103
  # Color of the anchor directory segments. Anchor segments are never shortened. The first
	# anchor 不会被截断，这是anchor的颜色
  # segment is always an anchor. 总是设置第一个分段为 anchor
  typeset -g POWERLEVEL9K_DIR_ANCHOR_FOREGROUND=39
  # Display anchor directory segments in bold.
	# 粗体显示 anchors 的颜色
  typeset -g POWERLEVEL9K_DIR_ANCHOR_BOLD=true
  # Don't shorten directories that contain any of these files. They are anchors.
	# 不截断包含这些文件的目录，把他们设置成 anchor
  local anchor_files=(
    .bzr
    .citc
    .git
  ...
  function my_git_formatter() {
    ...
    if [[ -n $P9K_CONTENT ]]; then
      ...
      typeset -g my_git_format="${1+%B%4F}(${1+%2F} ${1+%1F}"
      ...
      if (( VCS_STATUS_NUM_CONFLICTED )); then
			...
			# 有未最追踪的文件
				...
				my_git_format+=" ${1+%1F}❓"
			# 有 unstage shanges ，需要 add
			elif ((VCS_STATUS_NUM_STAGED)); then
				my_git_format+=" ${1+%3F}✘ "
			# 有stage changes 未提交，需要commit
